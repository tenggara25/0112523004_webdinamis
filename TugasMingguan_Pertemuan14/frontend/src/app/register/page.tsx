      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-2xl p-8 shadow-2xl shadow-black/20">
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <div>
              <label htmlFor="role" className="block text-sm font-medium text-text-muted mb-2">Role</label>
              <select id="role" value={role} onChange={(e) => setRole(e.target.value)} required
                className="w-full px-4 py-3 bg-input-bg border border-input-border rounded-xl text-foreground cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent">
                <option value="viewer">Viewer (Hanya Lihat)</option>
                <option value="operator">Operator (Tambah & Edit)</option>
